                ...
                if curP != '-1' and curP != self.curP:
                    ...
                    if self.all_ball_needToBetList:
                        ...
                        if self.console.isQQG == '0':
                            ...
                            self.all_ball_needToBetList = filter(lambda x:x[2] != 0 and x[2] != len(self.balls_bet_amount),b)
                        else:
                            # 常规 - 重置那些被爆了的
                    ...
                    # 写数据到Table 通知控制台下注
                    QMetaObject.invokeMethod(self.console, "loadTableData", Qt.QueuedConnection,Q_ARG(str,curP), Q_ARG(list, self.all_ball_needToBetList),Q_ARG(list, self.balls_bet_flag),Q_ARG(list, self.change_flag))
                    QMetaObject.invokeMethod(self.console, "onBet", Qt.QueuedConnection)
        except Exception, ex:
